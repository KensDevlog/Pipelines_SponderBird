...
public class FirebaseManagerTests
{
    ...
    {
        string json = BuildAuthJson("1234", "", "X", "project");
        firebaseManager.OnAuthReceived(json);
        Assert.IsFalse(firebaseManager.IsAuthenticated);
    }
    ...
    public void OnAuthReceived_CalledTwice_OverwritesFirstAuth()
    {
        ...
        Assert.AreEqual("token_4321", firebaseManager.IdToken);
        Assert.AreEqual("Spencer", firebaseManager.DisplayName);
        Assert.AreEqual("bug-sphere", firebaseManager.ProjectId);
    }

    [Test]
    public void SubmitScore_WhenNotAuthenticated_DoesNotThrow()
    {
        // encapsulating in an anon func to check if Submit Score will break, checking it safely
    ...
    {
        return
            $"{{\"uid\":\"{uid}\",\"idToken\":\"{idToken}\",\"displayName\":\"{displayName}\",\"projectId\":\"{projectId}\"}}";
    }
}
